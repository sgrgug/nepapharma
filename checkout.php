...
<?php
										if(isset($_POST['checkoutsub'])){
													$sql_1 = "SELECT * FROM my_carts WHERE email = '$semail'";
													...
													if($count_1>0){
														while($row_1=mysqli_fetch_assoc($res_1)){
															...
															$orEmail = $row_1['email'];
															$orProdN = $row_1['productN'];
															$orPrice = $row_1['price'];
															$orQty = $row_1['quantity'];
															$orTot = $row_1['tot'];
															$orImage = $row_1['iimage'];
															$orCheckStatus = $row_1['checkstatus'];

															
															mysqli_query($conn,"INSERT INTO `order` (`id`, `email`, `productN`, `price`, `quantity`, `tot`, `iimage`, `checkstatus`) VALUES (NULL, '$orEmail', '$orProdN', '$orPrice', '$orQty', '$orTot', '$orImage', '$orCheckStatus')");
															mysqli_query($conn,"DELETE FROM `my_carts` WHERE `email`='$semail'");
														}
													}
										}				
									?>